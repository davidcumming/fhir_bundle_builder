...
**Current Focus:** Implement the next bounded follow-on after medications-only multiplicity now has authoritative first-two-item mapping, placeholder-specific provenance, and stronger sibling-medication alignment while allergies and problems remain fixed at one.

## 9. Next Planned Slice

**Next Planned Slice:** Decide whether allergies/problems should remain fixed while the hardened bounded medications-only multiplicity path matures, or deepen medication-specific standards alignment for the multi-entry medications section.
...
- bounded medications-only multiplicity now has explicit normalized sibling-resource provenance, but broader multi-entry support for allergies, problems, or arbitrary medication counts would still require coordinated changes across planning, construction, validation, and repair
...
The immediate next objective is to decide whether allergies/problems should remain fixed while the hardened bounded medications-only path matures, or to deepen medication-specific standards alignment for the multi-entry medications section.
